...
testRecords (void)
{
    ...
    Value *val2;
    val2 = stringToValue("i1");
    OP_TRUE(val2, value, valueEquals, "first attr");
    freeVal(val2);
    freeVal(value);

    getAttr(r, schema, 1, &value);
    val2 = stringToValue("saaaa");
    OP_TRUE(val2, value, valueEquals, "second attr");
    freeVal(val2);
    freeVal(value);

    getAttr(r, schema, 2, &value);
    val2 = stringToValue("i3");
    OP_TRUE(val2, value, valueEquals, "third attr");
    freeVal(val2);
    freeVal(value);

    //modify attrs
    Value *val3 = stringToValue("i4");
    setAttr(r, schema, 2, val3);
    getAttr(r, schema, 2, &value);
    val2 = stringToValue("i4");
    OP_TRUE(val2, value, valueEquals, "third attr after setting");
    freeVal(val2);
    freeVal(val3);
    freeVal(value);
...
testCreateTableAndInsert (void)
{
    ...
    {   TEST_CHECK(createRecord(&r, schema));  // added Fall 2021
        ...
        Record *r2 = fromTestRecord(schema, inserts[pos]);
...
testUpdateTable (void)
{
    ...
    for(i = 0; i < numFinal; i++)
    {
        RID rid = rids[i];
        TEST_CHECK(getRecord(table, rid, r));
        Record *r2 = fromTestRecord(schema, finalR[i]);
        ASSERT_EQUALS_RECORDS(r2, r, schema, "compare records");
        freeRecord(r2);
    }
...
testInsertManyRecords(void)
{
    ...
    Record *r2 = fromTestRecord(schema, updates[0]);
    ASSERT_EQUALS_RECORDS(r2, r, schema, "compare records");
    freeRecord(r2);
...
void testScans (void)
{
    ...
    while((rc = next(sc, r)) == RC_OK)
    {
        ...
        {
            r2 = fromTestRecord(schema, scanOneResult[i]);
            if (memcmp(r2->data,r->data,getRecordSize(schema)) == 0)
                foundScan[i] = TRUE;
            freeRecord(r2);
        }
    }
...
void testScansTwo (void)
{
    ...
    while((rc = next(sc, r)) == RC_OK)
    {
        r2 = fromTestRecord(schema, inserts[1]);
        ASSERT_EQUALS_RECORDS(r2, r, schema, "compare records");
        freeRecord(r2);
    }
    if (rc != RC_RM_NO_MORE_TUPLES)
    ...
    while((rc = next(sc, r)) == RC_OK)
    {
        r2 = fromTestRecord(schema, inserts[5]);
        ASSERT_EQUALS_RECORDS(r2, r, schema, "compare records");
        c = serializeRecord(r, schema);
        free(c);
        freeRecord(r2);
    }
    if (rc != RC_RM_NO_MORE_TUPLES)
    ...
    while((rc = next(sc, r)) == RC_OK)
    {
        ...
        for(i = 0; i < numInserts; i++)
        {
            r2 = fromTestRecord(schema, inserts[i]);
            if (memcmp(r2->data,r->data,getRecordSize(schema)) == 0)
                foundScan[i] = TRUE;
